...
const Navbar = () => {
  ...
  const navLinks = (
    <>
      ...
      <li>
        <NavLink to="/my-profile" className={linkStyle}>
          My Profile
        </NavLink>
      </li>
      <li>
  ...
  return (
    <div className="sticky top-0 z-100 w-full bg-transparent">
      ...
      <div
        className={`navbar px-6 transition-all duration-500
        ${
          scrolled
            ? "border border-white/10 bg-[#0f172a]/40 backdrop-blur-xl shadow-[0_8px_32px_0_rgba(0,0,0,0.4)]"
            : "border border-white/5 bg-[#020617]/90 backdrop-blur-md"
        }`}
      >
        <div className="navbar-start">
          <div className="dropdown">
            ...
            <ul
              tabIndex={0}
              className="menu menu-sm dropdown-content mt-4 z-50 p-4 shadow-2xl bg-[#0f172a]/95 backdrop-blur-2xl rounded-3xl w-60 border border-white/10 text-white gap-3 leading-loose"
            >
              {navLinks}
            </ul>
          </div>
          ...
          <Link
            to="/"
            className="flex items-center gap-2 group transition-all ml-2 md:ml-0"
          >
        ...
        <div className="navbar-end gap-4">
          {user && (
            <div
              className="tooltip tooltip-bottom before:bg-cyan-600 before:text-white"
          ...
          {user ? (
            ...
            <div className="flex gap-3">
              <Link
                to="/login"
                className="btn btn-sm rounded-2xl bg-cyan-500 hover:bg-cyan-400 text-[#0f172a] border-none px-6 font-bold shadow-lg shadow-cyan-500/20 transition-all active:scale-95"
              >
                Login
              </Link>
              <Link
                to="/register"
                className="btn btn-sm rounded-2xl bg-white/5 border border-white/10 text-white hover:bg-white/10 px-6 hidden sm:flex font-bold transition-all"
              >
                Register
              </Link>
            </div>
